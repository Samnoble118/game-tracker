...
final readonly class SqliteCollectionMetadataRepository implements CollectionMetadataRepository
{
    ...
    public function __construct(private PDO $connection)
    {
        $this->connection->exec("CREATE TABLE IF NOT EXISTS collection_metadata (
            ...
            franchise TEXT NOT NULL DEFAULT '', characters TEXT NOT NULL DEFAULT '', location TEXT NOT NULL DEFAULT '',
    ...
    public function save(CollectionMetadata $metadata): void
    {
        ...
        $statement = $this->connection->prepare("INSERT INTO collection_metadata
            (item_type,item_id,user_id,franchise,characters,location,item_condition,item_packaging,purchase_price_pence,currency,purchased_on,retailer,serial_number,receipt_reference,private_notes,updated_at)
            VALUES (:item_type,:item_id,:user_id,:franchise,:characters,:location,:item_condition,:item_packaging,:purchase_price_pence,:currency,:purchased_on,:retailer,:serial_number,:receipt_reference,:private_notes,CURRENT_TIMESTAMP)
            ON CONFLICT(item_type,item_id,user_id) DO UPDATE SET franchise=excluded.franchise,characters=excluded.characters,
            location=excluded.location,item_condition=excluded.item_condition,item_packaging=excluded.item_packaging,purchase_price_pence=excluded.purchase_price_pence,
    ...
    private function parameters(CollectionMetadata $metadata): array
    {
        return [
            ...
            'franchise'=>$metadata->franchise(),'characters'=>$metadata->characters(),'location'=>$metadata->location(),
    ...
    private function hydrate(array $row): CollectionMetadata
    {
        return new CollectionMetadata(
            ...
            (string)$row['franchise'],(string)$row['characters'],(string)$row['location'],ItemCondition::from((string)$row['item_condition']),ItemPackaging::from((string)$row['item_packaging']),
