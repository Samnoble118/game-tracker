...
<?php elseif ($relatedTotal === 0): ?><div class="empty-state"><span>◇</span><h3>No related records yet</h3><p>Other items with this franchise will appear here.</p></div>
<?php else: ?><p class="result-count related-count"><?= $relatedTotal ?> connected <?= $relatedTotal === 1 ? 'item' : 'items' ?> · page <?= $relatedPage ?> of <?= $relatedTotalPages ?></p><div class="related-list"><?php foreach ($relatedItems as $relatedItem): ?><a href="<?= $escape($relatedItem['url']) ?>"><span><?= $escape($relatedItem['type']) ?></span><strong><?= $escape($relatedItem['name']) ?></strong><small><?= $escape($relatedItem['subtitle']) ?></small></a><?php endforeach; ?></div><?php if ($relatedTotalPages > 1): ?><nav class="pagination related-pagination" aria-label="Connected collection pages"><?php if ($relatedPage > 1): ?><a href="/?<?= $escape(http_build_query(['route'=>'collection-details','type'=>$subject['type']->value,'id'=>$subject['id'],'related_page'=>$relatedPage-1])) ?>">← Previous</a><?php endif; ?><span><?= $relatedPage ?> / <?= $relatedTotalPages ?></span><?php if ($relatedPage < $relatedTotalPages): ?><a href="/?<?= $escape(http_build_query(['route'=>'collection-details','type'=>$subject['type']->value,'id'=>$subject['id'],'related_page'=>$relatedPage+1])) ?>">Next →</a><?php endif; ?></nav><?php endif; ?><?php endif; ?></aside>
</main></body></html>
