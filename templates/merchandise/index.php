...
<body>
    ...
    <nav class="collection-tabs merchandise-collections" aria-label="Merchandise collections">
        <?php foreach (['all'=>'All items','owned'=>'Owned','wishlist'=>'Wishlist'] as $key => $label): ?>
            <a class="collection-tab <?= $activeCollection === $key ? 'is-active' : '' ?>" href="/?<?= $escape(http_build_query(['route'=>'merchandise','collection'=>$key,'category'=>$activeCategory,'q'=>$search])) ?>"><span><?= $label ?></span><strong><?= $counts[$key] ?></strong></a>
        <?php endforeach; ?>
    </nav>
    ...
    <section class="library-tools merchandise-tools">
        ...
        <nav class="platform-tabs" aria-label="Merchandise categories">
            <a class="platform-tab <?= $activeCategory === 'all' ? 'is-active' : '' ?>" href="/?<?= $escape(http_build_query(['route'=>'merchandise','collection'=>$activeCollection,'category'=>'all','q'=>$search])) ?>">All categories <strong><?= $categoryCounts['all'] ?></strong></a>
            <?php foreach ($categories as $category): ?>
                <a class="platform-tab <?= $activeCategory === $category->value ? 'is-active' : '' ?>" href="/?<?= $escape(http_build_query(['route'=>'merchandise','collection'=>$activeCollection,'category'=>$category->value,'q'=>$search])) ?>"><?= $escape($category->label()) ?><strong><?= $categoryCounts[$category->value] ?? 0 ?></strong></a>
            <?php endforeach; ?>
        </nav>
    </section>
    ...
    <main class="layout merchandise-layout">
        <section class="panel form-panel">
            ...
            <form class="game-form" method="post" action="/?route=merchandise">
                ...
                <div class="form-row"><label><span>Category</span><select name="category"><?php foreach ($categories as $category): ?><option value="<?= $category->value ?>" <?= $form['category'] === $category->value ? 'selected' : '' ?>><?= $escape($category->label()) ?></option><?php endforeach; ?></select></label><label><span>Collection</span><select name="collection_type"><?php foreach ($collectionTypes as $type): ?><option value="<?= $type->value ?>" <?= $form['collection_type'] === $type->value ? 'selected' : '' ?>><?= ucfirst($type->value) ?></option><?php endforeach; ?></select></label></div>
        ...
        <section class="panel collection-panel">
            ...
            <?php if ($items === []): ?><div class="empty-state"><span>＋</span><h3>No merchandise here yet</h3><p>Add a collectible or adjust the filters.</p></div><?php else: ?><div class="merchandise-grid">
                <?php foreach ($items as $item): ?><article class="merchandise-card">
                    ...
                    <div class="game-card-topline"><div class="card-badges"><span class="collection-badge collection-<?= $item->collectionType()->value ?>"><?= ucfirst($item->collectionType()->value) ?></span><span class="status"><?= $escape($item->category()->label()) ?></span></div><a class="text-link" href="/?route=merchandise&amp;edit=<?= $item->id() ?>">Edit</a></div>
